...
class ResourceMap {
  ...
  *[Symbol.iterator]() {
  ...
  load(raml) {
    for (let resource of raml.allResources()) {
      // TODO: Exchange {id} -> :id
...
module.exports = new ResourceMap();
...
Resource.create = function (resource, method) {
  ...
  klass.prototype.getSecuredBy = function() {
    const securedBy = method.securedBy().length
      ? method.securedBy().map(s => {
          // securedBy が小文字でないとpassportの登録ができない
          // securedBy にはnull が指定されることもある
          return s.name() ? s.name().toLowerCase() : s.name();
        })
      : [];
    return securedBy;
  }
...
function createResponseHandler(method) {
  ...
  const data = body.example() ? JSON.parse(body.example().value()) : null;
...
function createRequestHandler(method) {
  ...
  const type = types.get(body.type()[0]);
