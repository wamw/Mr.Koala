...
const registerStrategyFunctions = {
  ...
  'x-jwt': function(security) {
    const opts = {};
    opts.jwtFromRequest = jwt.ExtractJwt.fromAuthHeader(); // default AuthHeader
    if (security.describedBy &&
      security.describedBy.queryParameters) {
      for (let key in security.describedBy.queryParameters) {
